...
using InvKeyTree = KDTreeVectorOfVectorsAdaptor< KeyMat, float >; //第一个参数是存放数据变量的类型，第二个参数是：点坐标的类型
//类模板
...
{
public: 
    SCManager( ) = default; // reserving(保存) data space (of std::vector) could be considered. but the descriptor is lightweight so don't care.
...
public:
    ...
    std::vector<Eigen::MatrixXd> polarcontexts_; //存放所有的scan-context
    std::vector<Eigen::MatrixXd> polarcontext_invkeys_; //存放所有的scan-context每行均值，ring key,列向量(Nr*1) 
    std::vector<Eigen::MatrixXd> polarcontext_vkeys_; //存放所有的scan-context每列均值，sector key,行向量(1*Ns)

    KeyMat polarcontext_invkeys_mat_; //polarcontext_invkeys_，只不过每个元素不是eigen向量，是以向量形式存放
